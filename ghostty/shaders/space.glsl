...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // centeredUV length at a corner — makes the edge fade aspect-independent
    ...
    for (int i = 0; i < LAYERS; i++) {
        ...
        // fade out stars as they fly past the edges (relative to frame size)
        float edgeFade = 1.0 - smoothstep(0.75, 1.0, length(centeredUV) / maxR);
